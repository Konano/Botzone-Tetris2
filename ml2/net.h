...
public:
	...
	inline void output()
	{
		...
		rp(o,12) rp(i,3) rp(j,3) printf("%.9Lf ", w0[o][i][j]);
		rp(k,7) rp(o,12) rp(i,3) rp(j,3) printf("%.9Lf ", w1[k][o][i][j]);
		rp(i,56) rp(j,8) printf("%.9Lf ", w2[i][j]);
		rp(i,8) rp(j,8) printf("%.9Lf ", w3[i][j]);
		rp(i,8) printf("%.9Lf ", w4[i]);
		
		rp(i,12) printf("%.9Lf ", b0[i]);
		rp(i,7) printf("%.9Lf ", b1[i]);
		rp(i,8) printf("%.9Lf ", b2[i]);
		rp(i,8) printf("%.9Lf ", b3[i]);
		printf("%.9Lf ", b4);
	...
	inline void trainning(long double aim)
	{
		...
		rp(o,7) rp(i,3) rp(j,8) da1[o][i][j] = (a1[o][i][j]==c1[o][j] ? dc1[o][j] : 0);
		...
		rp(o,12) rp(i,10) rp(j,20) da0[o][i][j] = (a0[o][i][j]==c0[o][i/2][j/2] ? dc0[o][i/2][j/2] : 0);
	...
	/* inline void g(long double aim)
	{
		...
		rp(o,7) rp(j,8) dc1[o][j] = di1[56*t+8*o+j];
		rp(o,7) rp(i,3) rp(j,8) da1[o][i][j] = (a1[o][i][j]==c1[o][j] ? dc1[o][j] : 0);
		...
		{
			gw0[o][x][y]=0;
			rp(i,10) rp(j,20) gw0[o][x][y]+=input0[i+x][j+y]*da0[o][i][j];
		}
		rp(o,7) rp(k,12) rp(x,3) rp(y,3)
		{
			gw1[o][k][x][y]=0;
			rp(i,3) rp(j,8) gw1[o][k][x][y]+=c0[k][i+x][j+y]*da1[o][i][j];
		}
		rp(x,56) rp(y,8) gw2[x][y]=input1[x]*-da2[y];
		rp(x,8) rp(y,8) gw3[x][y]=a2[x]*-da3[y];
		rp(x,8) gw4[x]=a3[x]*-da4;
		
		rp(o,12)
		{
			gb0[o]=0;
			rp(i,10) rp(j,20) gb0[o]+=da0[o][i][j];
		}
		rp(o,7)
		{
			gb1[o]=0;
			rp(i,3) rp(j,8) gb1[o]+=da1[o][i][j];
		}
		rp(y,8) gb2[y]=-da2[y];
		rp(y,8) gb3[y]=-da3[y];
		gb4=-da4;
	...
	inline bool test(long double aim)
	{
		...
		rp(x,8) if (check(w4[x],gw4[x])) {puts("w4"); return false;}
		rp(y,8) if (check(b3[y],gb3[y])) {puts("b3"); return false;}
		rp(x,8) rp(y,8) if (check(w3[x][y],gw3[x][y])) {puts("w3"); return false;}
		rp(y,8) if (check(b2[y],gb2[y])) {puts("b2"); return false;}
		rp(x,56) rp(y,8) if (check(w2[x][y],gw2[x][y])) {puts("w2"); return false;}
		rp(o,7) if (check(b1[o],gb1[o])) {puts("b1"); return false;}
		rp(o,12) if (check(b0[o],gb0[o])) {puts("b0"); return false;}
		rp(o,12) rp(x,3) rp(y,3) if (check(w0[o][x][y],gw0[o][x][y])) {puts("w0"); return false;}
		rp(o,7) rp(k,12) rp(x,3) rp(y,3) if (check(w1[o][k][x][y],gw1[o][k][x][y])) {puts("w1"); return false;}
